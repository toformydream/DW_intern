...
def main():
    file_list = ['art_knife', 'chief_knife', 'fruit_knife', 'jack_knife', 'steak_knife', 'swiss_army_knife','office_knife']
    img_amount = 0
...
def amend_json(json_path, files_path, imgs_amount):
    ...
    number = files_count(files_path)
    print(number)
    for a in range(number):
        print(a)
        json_data['images'][a]['id'] = json_data['images'][a]['id'] + imgs_amount
        json_data['images'][a]['path'] = 'D:/wp/DW_intern/knife_data_file' + str(json_data['images'][a]['id']+1) + '.png'
...
def resize_upload_image(file_path, json_data, json_dic, img_amount,
        ...
        for o in range(0, 3):
            ...
            for i in range(0, 2):  # png파일하나당 3개씩 만들기
                ...
                for seg_len in range(0, len(
                        json_data['annotations'][json_dic[str(j + add_number+img_amount) + '.png']]['segmentation'][0][0])):
                    if seg_len % 2 == 0:  ##############################################################################################################
                        new_seg.append(
                            json_data['annotations'][json_dic[str(j + add_number+img_amount) + '.png']]['segmentation'][0][0][
                                seg_len] * new_width)
                ...
                plus_number += 1
    with open('json/knife_data.json', "w", encoding="UTF-8") as file:
        json.dump(json_data, file, indent=4)
